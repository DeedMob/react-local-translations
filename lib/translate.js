...
function translate(_a) {
    var locale = _a.locale, translations = _a.translations, _b = _a.globalTranslations, globalTranslations = _b === void 0 ? {} : _b, convertMissingKey = _a.convertMissingKey, _c = _a.transforms, transforms = _c === void 0 ? {} : _c, preprocess = _a.preprocess, postprocess = _a.postprocess;
    ...
    function usingTranslations(trs) {
        function tr(key, interpolation) {
            if (preprocess) {
                var preprocessedKey = preprocess(key, trs);
                if (preprocessedKey)
            ...
            result = result.replace(TOKEN_REGEX, function (_, expr) {
                ...
                if ((inlineTranslationType === 't' || inlineTranslationType === 'g') &&
                    expr[1] === ':') {
                    replacement = usePhrase(inlineTranslationType === 't' ? translations : globalTranslations, transformChain[0].substr(2));
                }
                if (!replacement) {
                    var nestedObjectPropertyPath = transformChain[0].split('.');
                    replacement = nestedObjectPropertyPath.reduce(function (accum, next) {
